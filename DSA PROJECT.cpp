...
void bank::insertData() // open account
{
    ...
    // Validate and store the account number as a numeric value
    while (true) {
        cout << "\nEnter new account Number of 8-9 digits: ";
        string accountInput;
        cin >> accountInput;

        if (accountInput.length() >= 8 && accountInput.length() <= 9 && std::all_of(accountInput.begin(), accountInput.end(), ::isdigit)) {
            long long id = stoll(accountInput);
            ...
            if (!isDuplicateID) {
                temp->id = id; // Store the account number
                break; // Valid account ID, exit the loop
            }
        } else {
    ...
    // Enter the cash amount for the account
    ...
    cout << "Account Number: " << temp->id << endl; // Display the account ID
}
...
void pattern(){
    setColor(3);
cout <<          "__________    _____    _______   ____  __.______________._.\n"
                 "\\______   \\  /  _  \\   \\      \\ |    |/ |   \\_    ___/| |\n"
                 " |    |  _/ /  /_\\  \\  /   |   \\|      < |   | |    |   | |\n"
                 " |    |   \\/    |    \\/    |    \\    |  \\|   | |    |    \\|\n"
